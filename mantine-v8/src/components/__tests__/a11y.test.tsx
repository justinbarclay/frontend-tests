...
// Mantine components require MantineProvider in the tree
async function renderWithMantine(ui: React.ReactElement) {
  const result = render(<MantineProvider>{ui}</MantineProvider>);
  // Flush async state updates from Mantine internals (e.g. Popover)
  await act(async () => {});
  return result;
}
...
describe("ConfigurationForm", () => {
  test("text type", async () => {
    ...
    const { container } = await renderWithMantine(<ConfigurationForm />);
...
describe("PreviewField", () => {
  test("text field", async () => {
    ...
    const { container } = await renderWithMantine(<Preview />);
    await checkA11y(container);
  });
